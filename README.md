...
A separate Colab notebook can run NLRexpress and prepare optional `name,sequence,aa_change` batch input files for CodonDomesticate, keeping MHD candidate metadata and warning in the optional batch-table step when MHD probability is below a configurable threshold.
